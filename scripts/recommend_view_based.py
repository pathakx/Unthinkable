...
import faiss
import pandas as pd
from .shared_utils import build_faiss_index, compute_event_probabilities, EVENT_WEIGHTS

def recommend_view_based(user_id, interactions_df, product_embeddings, product_ids, product_id_to_index, top_k=2):
    ...
    user_df = user_df[user_df["event_type"] == "view"]
    ...
    for pid in user_df.sort_values("probability", ascending=False)["product_id"].head(3):
        ...
        for i, s in zip(I[0], D[0]):
            ...
            if candidate_id != pid and candidate_id not in seen:
                recs.append({"product_id": candidate_id, "score": float(s), "source_event": "view"})
