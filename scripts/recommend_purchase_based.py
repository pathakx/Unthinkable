...
def recommend_purchase_based(user_id, interactions_df, product_embeddings, product_ids, product_id_to_index, top_k=5):
    """Cross-sell recommendations based on past purchases."""
    user_df = interactions_df[interactions_df["user_id"] == user_id]
    user_df = compute_event_probabilities(user_df)
    user_df = user_df[user_df["event_type"] == "purchase"]
    ...
    for pid in user_df.sort_values("probability", ascending=False)["product_id"].head(3):
        ...
        for i, s in zip(I[0], D[0]):
            ...
            if candidate_id != pid and candidate_id not in seen:
                recs.append({"product_id": candidate_id, "score": float(s), "source_event": "purchase"})
